...
class IDD3DCategoryConverter(BaseConverter):
    ...
    def run(self, data_loader: IDD3DDataLoader, log_handler: LogHandler):
        ...
        # IDD3D actual object types - verified from dataset
        # These are the exact class names used in IDD3D label files
        idd3d_to_nuscenes_categories = {
            ...
            'animal': 'animal',
            'traffic light': 'static_object.traffic_light',
            'traffic sign': 'static_object.traffic_sign',
            'pole': 'static_object.pole',
            'other vehicle': 'vehicle.other',
        ...
        log_handler.log(f"Scanning {len(frame_ids)} frames for object types...", 'info')
            ...
            try:
                ...
                for obj in label_objects:
                    ...
                    if obj_type:
                        ...
                        unique_obj_types.add(obj_type.lower())
            except Exception as e:
                log_handler.log(f"Error reading {label_path}: {str(e)}", 'warning')
        
        log_handler.log(f"Found {len(unique_obj_types)} unique object types: {sorted(unique_obj_types)}", 'info')
        ...
        for obj_type in sorted(unique_obj_types):
            ...
            category_name = idd3d_to_nuscenes_categories.get(
                obj_type, 
                f'movable_object.{obj_type.replace(" ", "_")}'
            )
            ...
            log_handler.log(f"  Created category: {obj_type} → {category_name} (token: {category['token']})", 'info')
...
class IDD3DInstanceConverter(BaseConverter):
    """Generate instance.json from label files and category.json"""
    ...
    def run(self, data_loader: IDD3DDataLoader, log_handler: LogHandler):
        ...
        if not os.path.exists(category_path):
            log_handler.log("category.json not found. Run category converter first.", 'error')
            return
        ...
        try:
            ...
            log_handler.log(f"Loaded {len(categories)} categories", 'info')
        ...
        # IDD3D to nuScenes category mapping (must match CategoryConverter)
        idd3d_to_nuscenes_categories = {
            'car': 'vehicle.car',
            'truck': 'vehicle.truck',
            'bus': 'vehicle.bus',
        ...
        annot_data = data_loader.read_annotations()
        if not annot_data:
            log_handler.log("No annotations found", 'error')
            return
        
        frame_ids = sorted(annot_data.keys())
        instance_tracker = {}
        
        log_handler.log(f"Processing {len(frame_ids)} frames for instances...", 'info')
        
        # Collect all objects and their appearances across frames
        for frame_id in frame_ids:
            label_path = os.path.join(data_loader.label_dir, f"{frame_id}.json")
            if not os.path.exists(label_path):
                continue
            ...
            try:
                ...
                for obj in label_objects:
                    ...
                    # Normalize object type to lowercase
                    obj_type_normalized = obj_type.lower()
                    
                    # Create new instance if first time seeing this obj_id
                    if obj_id not in instance_tracker:
                        instance_token = uuid.uuid4().hex
                        
                        # Map object type to category name
                        category_name = idd3d_to_nuscenes_categories.get(
                            obj_type_normalized,
                            f'movable_object.{obj_type_normalized.replace(" ", "_")}'
                        )
                        
                        # Get category token from category.json
                        ...
                        instance_tracker[obj_id] = {
                            ...
                            'last_frame': frame_id,
                            'first_annotation_token': f"ann_{frame_id}_{obj_id}",
                            'last_annotation_token': f"ann_{frame_id}_{obj_id}"
                        }
                    else:
                        # Update last frame for existing instance
                        instance_tracker[obj_id]['last_frame'] = frame_id
                        instance_tracker[obj_id]['last_annotation_token'] = f"ann_{frame_id}_{obj_id}"
                        
            except Exception as e:
                log_handler.log(f"Error processing {label_path}: {str(e)}", 'warning')
        ...
        for obj_id, data in instance_tracker.items():
            instance = {
                ...
                "first_frame_token": data['first_frame'],
                "last_frame_token": data['last_frame']
            }
        ...
        # Log statistics
        log_handler.log(f"Instance breakdown:", 'info')
...
class IDD3DObjectConverter(BaseConverter):
    """Generate objects.json with 3D bounding box annotations"""
    ...
    def run(self, data_loader: IDD3DDataLoader, log_handler: LogHandler):
        ...
        if not os.path.exists(frames_path):
            log_handler.log("frames.json not found. Run annotation converter first.", 'error')
            return
        ...
        try:
            ...
            log_handler.log(f"Error reading frames.json: {str(e)}", 'error')
            return
        ...
        if os.path.exists(category_path):
            try:
                ...
                log_handler.log(f"Error reading category.json: {str(e)}", 'warning')
        
        # Read instance.json to get instance tokens
        ...
        if os.path.exists(instance_path):
            try:
                ...
                # Build reverse mapping from annotation tokens to instance tokens
                for inst in instances:
                    # Extract obj_id from annotation token pattern: ann_frameId_objId
                    first_ann = inst['first_annotation_token']
                    parts = first_ann.split('_')
                    if len(parts) >= 3:
                        obj_id = '_'.join(parts[2:])
                        obj_to_instance[obj_id] = inst['instance_token']
            except Exception as e:
                log_handler.log(f"Error reading instance.json: {str(e)}", 'warning')
        ...
        # Process each frame and create object annotations
        objects = []
        ...
        for frame in frames:
            ...
            for obj in frame_objs:
                ...
                # Get instance token
                instance_token = obj_to_instance.get(obj_id, uuid.uuid4().hex)
                ...
                object_token = f"obj_{frame_id}_{obj_id}"
                
                # Extract 3D bounding box center
                center = [
                    position.get('x', 0.0),
                    position.get('y', 0.0),
                    position.get('z', 0.0)
                ...
                # Extract size (width, length, height)
                size = [
                    scale.get('x', 1.0),
                    scale.get('y', 1.0),
                    scale.get('z', 1.0)
                ...
                # Simple euler to quaternion (Z-Y-X order)
                cy = math.cos(rz * 0.5)
                sy = math.sin(rz * 0.5)
                cp = math.cos(ry * 0.5)
                sp = math.sin(ry * 0.5)
        ...
        # Save objects.json
        out_path = os.path.join(data_loader.annot_out, 'objects.json')
        with open(out_path, 'w') as f:
            json.dump(objects, f, indent=2)
        
        log_handler.log(f"✓ Objects file created with {len(objects)} 3D annotations", 'success')
